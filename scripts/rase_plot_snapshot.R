...
kResGridHeight <- 0.045  # heigth of a resistance cell, fraction of the height of the graph above
kGridShift <- 2
...
CatToCatName <- function(cat) {
   catu <- toupper(cat)
   ifelse(catu == "S", "susceptible", (ifelse(catu == "R", "non-susceptible", "unknown")))
}
...
xx <- barplot(height = 0 * vals.to.plot - y, col = "#ffffff", border = NA, space = 0, axes = F, add = T)

axis(side = 2, pos = -0.2, cex.axis = 0.8, at = seq(0, maxtick, 0.01), mgp = c(50, 0.5, 0))
...
## taxid
text(y = 0, x = x, labels = sel$taxid, pos = 1, cex = 0.05, offset = 0.35, col = "#eeeeee")


abline(v = seq(0:kSelected), col = "white")

text(y = -kGridShift * 0.5 * AbsResGridHeight, x = 35, labels = "Isolate", cex = 1.75, col = "#000000")
...
# timestamp
mtext(side = 1, text = paste(as.POSIXct(timestamp, origin = "1970-01-01"), "  "), line = 0.8, adj = 1, cex = 0.95)
